...
def start_learning(train_path,test_path,lr,l1_size,l2_size,batch_size,epoch_num,lr_inh):
    ...
    if os.path.exists(save_model_name+'_loss.csv'):
        ...
        if lr_inh:
            lr = loss_df.iloc[-1]['lr']
            print("using learning rate from the record:",lr)
    ...
    for epoch in range(epoch_num):
        ...
        if not (epoch+1) % 50:
            ...
            temp_loss.to_csv('temp_'+save_model_name+'_'+'_loss.csv', index=False)
...
train_path = 'vectorized_train02.csv'
test_path = 'vectorized_test02.csv'
lr = 0.00001
l1_size = 256
l2_size = 64
batch_size = 1024
...
start_learning(train_path, test_path, lr, l1_size, l2_size, batch_size, epoch_num, lr_inh = True)
